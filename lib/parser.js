 ...
 */
function generateMetadataObject(sections) {
  let values = [];
  sections.forEach(function(section){
      sectionHeader = section.shift();
      section.forEach(function(value){
        value.section = sectionHeader.section;
...
function getValueType(value) {
  var type = null;
  if (value.value === 'nil') {
    //console.error(`nil value is not defined. Can't determine type of value: ${value.name}.`);
    //throw new Error(`nil value is not defined. Can't determine type of value: ${value.name}.`);
  } else {
    if(value.value === '') {
      type = 'string';
    } else if(value.value === '[]') {
