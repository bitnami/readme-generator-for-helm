 ...
 *   isPlainArray: bool // specifies if the parameter is a plain array (contains only strings)
 * }
 */
function createValuesObject(valuesFilePath) {
  ...
  for (let valuePath in  dot.dot(valuesJSON)) {
    ...
    if (valuePathSplit.length > 1) {
      ...
      const arrayPrefix = valuePathSplit[valuePathSplit.length - 2];
      let isPlain = true; // Assume it is plain until we prove the opposite
      _.get(valuesJSON, arrayPrefix).forEach((e) => {
        if (!(typeof e === "string")) {
           isPlain = false;
         }
      });
      isPlainArray = isPlain;
      if (isPlainArray) {
    ...
    if (typeof value === "object") {
      // The dot paths always finish at the latest element in the YAML tree, so
      // when the final value is an empty object we need to set it as '{}' to avoid getting '[object Object]'
      value = '{}';
    }
