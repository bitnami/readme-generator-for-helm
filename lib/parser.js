 ...
 * The function reads Chart.yaml file, goes over dependencies array,
 * sends axios GET request to the repository of the dependency chart, reads the home page,
 * and injects home page Parameter by finding any first dependency chart variable
 */
async function appendDependencies(dependencyPath, valuesMetadata) {
  const chart = YAML.parse(fs.readFileSync(dependencyPath, 'utf8'));

  axiosRetry(axios, {
    retries: 3,
    retryDelay: (retryCount) => {
      console.log(`INFO: retrying to connect to url, attempt ${retryCount}...`);
      return 0;
    }
  });
  ...
  console.log(`INFO: receiving dependencies home pages...`);
  for (const subChart of chart.dependencies) {
    const repositoryUrl = subChart.repository + "/index.yaml";

    // skip if its already defined
    if (valuesMetadata.find(param => param.name === subChart.name)) {
      continue;
    }

    // example: subChart.name == minio then search for minio.something
    const firstParam = valuesMetadata.find(param => {
      paramStartName = param.name.match(/([^.]+)[.]/);
      return paramStartName && paramStartName[1] === subChart.name;
    })

    // skip if didnt found first subchart description
    if (!firstParam) {
      continue;
    }

    const index = valuesMetadata.indexOf(firstParam);

    requests.push(axios.get(repositoryUrl).then((response) => {
      if (response.status !== 200) {
        throw new Error(`ERROR: couldnt connection failed to "${repositoryUrl}", return status code: ${err.response.status} after 3 retry attempts`);
      } else {
        const param =  new Parameter(subChart.name);
        const homeUrl = YAML.parse(response.data).entries[subChart.name][0].home;
        param.description = `For additional variables configurations please refer [here](${homeUrl})`;
        param.validate = false;

        param.section = firstParam.section;
  ...
  console.log(`INFO: successfully received dependencies home pages!`);
}
