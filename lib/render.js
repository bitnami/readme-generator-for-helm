...
function renderOpenAPISchema(schemaFilePath, parametersList) {
  ...
  const nilValues = paramsList.filter((p) => p.value === 'nil' && !utils.containsModifier(p, 'nullable')).map((p) => p.name);
  ...
  paramsList.forEach((p) => {
    if (p.value === 'nil' && utils.containsModifier(p, 'nullable')) {
  ...
  paramsList.forEach((p) => {
    if (utils.containsModifier(p, 'nullable')) {
      p.nullable = true;
    }
  });
  ...
  paramsList = paramsList.filter((p) => !utils.containsModifier(p, 'object'));
