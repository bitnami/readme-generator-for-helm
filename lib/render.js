...
export function insertReadmeTable(readmeFilePath, sections, config) {
  ...
  lines.forEach((line, i) => {
    ...
    const match = line.match(new RegExp(`^(##+) <a id="${config.regexp.paramsSectionTitle.key}"></a> ${config.regexp.paramsSectionTitle.title}`));
    if (match) {
      lineNumberSigns = match[1];
      paramsSectionLimits.push(i + 3); // Keep section's description
      console.log(`INFO: Found parameters section at line: ${i + 3}`);
    }
  });
