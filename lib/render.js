...
function renderOpenAPISchema(schemaFilePath, parametersList, config) {
  ...
  // The parameters should be filtered if they have nested properties defined
  // For example:
  // @param a.b [object] Whatever
  // a:
  //   b: "something"
  // "a.b" must be in the schema, while "a" is not an actual entry (Rendered in the README only)
  paramsList = paramsList.filter((p) => !paramsList.some(
    (other) => other.name.match(new RegExp(`^${p.name}(\\.|\\[\\d+\\])`)),
  ));
