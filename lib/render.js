...
export function insertReadmeTable(readmeFilePath, sections, config) {
  ...
  lines.forEach((line, i) => {
    // Find first parameters subsection start
    // use minimun two # symbols since just one is the README title
    if (!titleFound) {
      match = line.match(new RegExp(`^(##+)\\s+${config.regexp.paramsSectionTitle}`));
    } else if (paramsSectionLimits.length === 0) {
      match = line.match(new RegExp(`^${lineNumberSigns}#\\s+`));
    }
    if (match) {
      if (!titleFound) {
        titleFound = true;
        ...
        match = null; // reset match to avoid finding another section
